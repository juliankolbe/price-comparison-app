...
module.exports = (function () {
  ...
  function createTransformerCB (settings) {
    ...
    let lowerConversions = _.transform(conversions, (result, val, key) => { result[key.toLowerCase()] = val }, {})
    let fields = settings.fields || null
    let lowerFields = _.transform(fields, (result, val, key) => { result[key.toLowerCase()] = val }, {})
    let requiredHeadings = settings.requiredHeadings.map(heading => heading.toLowerCase()) || []
    let validHeadings = settings.validHeadings.map(heading => heading.toLowerCase()) || []
    let recordCounter = 0
    ...
    return function transformerCB (record, callback) {
      ...
      if (passed && recordCounter < 1) {
        ...
        // let validIntersection = _.intersection(csvHeadings, validHeadings)
