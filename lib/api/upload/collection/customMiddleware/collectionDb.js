...
const arrangePriceListData = require('../helpers/arrangePriceListData')
...
const collectionDb = (req, res, next) => {
  Promise.coroutine(function * () {
    ...
    yield bookshelf.transaction(Promise.coroutine(function * (t) {
      ...
      // Add price list ids to file objects
      files.forEach(file => {
        file.priceListId = priceListIds[file.fileName]
      })
      //
      ...
      let result = yield arrangePriceListData(files, { database: true })
      // logger.debug(result)
      if (result.filesHadErrors) {
        delete result.priceListDataObjs
      } else {
        _.forOwn(result.statsObj, (value, key) => {
          delete result.statsObj[key].errorReport
        })
        let priceListDataObjs = result.priceListDataObjs
      ...
      req.csvUploadResponse = result
      logger.info('DB Process took: ' + (new Date() - start) + 'ms')
    }))
  })()
  .then(() => {
    if (req.csvUploadResponse.fileHardErrors) {
      logger.info('DB Transaction rolled back due to record errors')
      // TODO: Error Report here i think
      next(new Error('DB Transaction rolled back due to record errors'))
    } else {
      logger.info('DB Transaction successful')
      // End request here by responding after a successful db transaction.
      res.json({ success: true, data: req.statsObj })
    }
  })
  .catch(err => {
