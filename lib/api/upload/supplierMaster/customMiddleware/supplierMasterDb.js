...
const supplierMasterDb = (req, res, next) => {
  Promise.coroutine(function * () {
    ...
    yield bookshelf.transaction(Promise.coroutine(function * (t) {
      ...
      yield masterProductNames.batchInsert(3000, {transacting: t})
      ...
      let masterIds = masterProductNames.getNameIdPairs('master_product_name', 'id')
      let supplierIds = suppliers.getNameIdPairs('name', 'id')
      // Due to changes in csv parsing, all headings are now lowercase, and so should the id fields be then
      supplierIds = _.transform(supplierIds, (result, val, key) => { result[key.toLowerCase()] = val }, {})
      let tree = {}
      ...
      yield supplierProductNames.batchInsert(3000, {transacting: t})
